...
impl ScanQueue {
    ...
    pub async fn pop(&self) -> Result<Option<ScanJob>> {
        ...
        for priority in [
            ...
            let result: Option<(String, f64)> = conn.zpopmin(&queue_key, 1).await.ok().and_then(
                |v: Vec<(String, f64)>| {
                    if v.is_empty() {
                        None
                    } else {
...
mod tests {
    ...
    async fn test_queue_push_pop() {
        ...
        let job = ScanJob::new("test-package".to_string(), Some("1.0.0".to_string()), ScanPriority::High);
        let job_id = job.id;
