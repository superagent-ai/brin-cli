...
pub async fn get_package(
    ...
    let threats = state.db.get_package_threats(package.id).await.map_err(|e| {
    ...
    // Build response
    let risk_reasons: Vec<String> = serde_json::from_value(package.risk_reasons.clone())
        .unwrap_or_default();
...
pub async fn get_package_version(
    ...
    // Fetch associated CVEs and threats
    let cves = state.db.get_package_cves(package.id).await.unwrap_or_default();
...
pub async fn scan_tarball(
    ...
    let tarball_data = tarball_data.ok_or_else(|| {
        (
            StatusCode::BAD_REQUEST,
            Json(json!({ "error": "No tarball file provided. Use field name 'tarball' or 'file'" })),
        )
    })?;
