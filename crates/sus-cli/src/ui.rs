...
use sus_common::{PackageResponse, RiskLevel};
...
fn print_critical_assessment(assessment: &PackageResponse) {
    ...
    for threat in &assessment.agentic_threats {
        if threat.confidence > 0.8 {
            let threat_desc = match threat.threat_type {
                sus_common::ThreatType::MaliciousCode => "malware detected",
                sus_common::ThreatType::PromptInjection => "prompt injection attack",
                sus_common::ThreatType::InstructionOverride => "instruction override attack",
                sus_common::ThreatType::DataExfiltration => "data exfiltration attempt",
                sus_common::ThreatType::SocialEngineering => "social engineering attack",
                sus_common::ThreatType::InstallScriptInjection => "malicious install script",
            };
    ...
    if assessment.risk_reasons.iter().any(|r| r.to_lowercase().contains("malware") || r.to_lowercase().contains("malicious")) {
...
pub fn print_capabilities(assessment: &PackageResponse) {
    ...
    if !caps.environment.accessed_vars.is_empty() {
        let vars: Vec<&str> = caps.environment.accessed_vars.iter().take(3).map(|s| s.as_str()).collect();
