...
pub async fn run(client: &SusClient, package: &str) -> Result<()> {
            ...
            if e.to_string().contains("not found") {
                ...
                match client.request_scan(name, version).await {
                    Ok(resp) => {
                        ...
                        println!();
                        println!("  run {} again in a moment.", format!("sus check {}", package).cyan());
                    }
                    Err(scan_err) => {
    ...
    println!(
        "  scanned: {}",
        assessment.scanned_at.format("%Y-%m-%d %H:%M UTC").to_string().dimmed()
    );

    // Final verdict
    println!();
    match assessment.risk_level {
        sus_common::RiskLevel::Clean => {
            println!("  {} This package appears safe to use.", "verdict:".green().bold());
        }
        sus_common::RiskLevel::Warning => {
            println!(
                "  {} Review the warnings above before using.",
                "verdict:".yellow().bold()
            );
        }
        sus_common::RiskLevel::Critical => {
            println!(
