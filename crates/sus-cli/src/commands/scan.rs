...
use std::path::Path;
use sus_common::{PackageResponse, PackageVersionPair, RiskLevel};
...
pub async fn run(client: &SusClient, json: bool) -> Result<()> {
    ...
    let assessments = client.bulk_lookup(&deps).await.unwrap_or_else(|e| {
        tracing::warn!("Bulk lookup failed: {}, falling back to individual lookups", e);
        vec![]
    });
    ...
    if !unknown.is_empty() {
        println!("📦 {} packages not yet scanned:", unknown.len().to_string().dimmed());
        for dep in &unknown {
            println!("   {} {}@{}", "?".dimmed(), dep.name, dep.version);
        }
        println!();
        println!(
            "   run {} to request scans",
            "sus check <package>".cyan()
        );
        println!();
    }
...
fn collect_lock_deps(deps: &serde_json::Map<String, serde_json::Value>, out: &mut Vec<PackageVersionPair>) {
