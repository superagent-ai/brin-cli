...
use sus_common::{AgenticThreatSummary, ApiDoc, ThreatType, UsageDocs};
...
If no threats found, return: {"threats": [], "summary": "No threats detected"}"#;
    ...
    pub async fn generate_usage_docs(
        ...
        tracing::debug!(
            "Generating usage docs for {} using OpenCode",
            package_name
        );
...
- Use modern ES6+ syntax in examples"#,
    ...
    fn parse_json_output<T: for<'de> Deserialize<'de> + Default>(&self, output: &str) -> Result<T> {
        ...
        tracing::debug!("Extracted text from OpenCode: {}", &text_content[..text_content.len().min(200)]);
        ...
        tracing::warn!("Failed to parse OpenCode text as JSON, using defaults. Text: {}", &text_content[..text_content.len().min(500)]);
        Ok(T::default())
    }
}
