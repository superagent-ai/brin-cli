...
use sus_common::{
    ...
    AgenticThreatSummary, CveSummary, Database, PackageCapabilities, RiskLevel, UsageDocs,
};
...
impl PackageScanner {
    ...
    pub async fn scan(&self, package: &str, version: Option<&str>) -> Result<ScanResult> {
        ...
        self.scan_extracted(package, &version, Some(&metadata), Some(&version_info), extracted).await
    }
    ...
    pub async fn scan_tarball(&self, tarball_path: &std::path::Path) -> Result<ScanResult> {
        ...
        let package = extracted.package_json
            .get("name")
            .and_then(|v| v.as_str())
            .ok_or_else(|| anyhow::anyhow!("No name in package.json"))?
            .to_string();

        let version = extracted.package_json
            .get("version")
            .and_then(|v| v.as_str())
            .unwrap_or("0.0.0")
            .to_string();

        self.scan_extracted(&package, &version, None, None, extracted).await
    }
    ...
    async fn scan_extracted(
        ...
        package: &str,
        version: &str,
        metadata: Option<&sus_common::NpmPackageMetadata>,
        version_info: Option<&sus_common::NpmVersionInfo>,
        ...
        let (cves, agentic_threats, capabilities, usage_docs) = tokio::join!(
            ...
            self.agentic_scanner.generate_usage_docs(&extracted, package)
        );
        ...
        let trust_score = self.calculate_trust_score(metadata, version_info);

        // 6. Determine risk level
        let (risk_level, risk_reasons) =
            self.calculate_risk(&cves, &agentic_threats, &capabilities, trust_score);
        ...
        let package_id = self
            ...
            .upsert_package(&NewPackage {
                ...
                maintainer_count: metadata.and_then(|m| m.maintainers.as_ref()).map(|m| m.len() as i32),
                last_publish: None,
    ...
    fn calculate_trust_score(
        &self,
        metadata: Option<&sus_common::NpmPackageMetadata>,
        _version_info: Option<&sus_common::NpmVersionInfo>,
    ) -> u8 {
    ...
    fn calculate_risk(
        ...
        for threat in agentic_threats {
            if threat.confidence > 0.8 {
                reasons.push(format!("{:?} detected ({}% confidence)", threat.threat_type, (threat.confidence * 100.0) as u8));
                max_level = RiskLevel::Critical;
            } else if threat.confidence > 0.5 {
                reasons.push(format!("Possible {:?} ({}% confidence)", threat.threat_type, (threat.confidence * 100.0) as u8));
                if max_level != RiskLevel::Critical {
                    max_level = RiskLevel::Warning;
                }
            }
